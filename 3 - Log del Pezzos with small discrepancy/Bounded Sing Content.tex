...
\]
We denote by $a_i$ the value $C_i^2$. The values $a_i$ uniquely define $S$ via Hirzebruch-Jung fractions \cite{ReidCyclic} and the notation $S=\frac{1}{r}(1, a) = [a_1, \dots , a_n]$ indicates the singularity such that the resolution has these self intersections.
\label{sec!notation}
...
\end{blockarray}
\]
Taking the Veronese embedding of degree $\left( \begin{array}{c} u \\ b \end{array} \right)$ we get a codimension 2 complete intersection with weights
...
\begin{figure}\label{CurveConfigFigure}[h]
\resizebox{10cm}{10cm}{
