...
\begin{algorithm}
We start with a singularity given by $a \otimes P_1  + b \otimes P_2 + c \otimes P_3$, such that the minimal resolution contains a parabolic line, (note we allow some of these values to be 0). 

We wish to bound theses values, if the singularity is of type $E$ then there are already only finitely many. If it is of type $A_n$ then we can assume $c = 0$ and $a = \frac{k}{u}$ and $b = \frac{k}{v}$. Assume that the fan on $P_1$ and $P_2$ both have only one point, then there are only finitely many options as, as $u$ and $v$ increase $-k$ modulo $u$ and $v$ also increase and this would increase the index of the singularity corresponding to the tail cone $[-\infty, 0)$. Hence this case is bounded. Alternatively assume there is another point on the fan of $P_1$. As $u$ and $v$ increase in value
