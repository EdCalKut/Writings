...
\begin{dfn}
Let $X$ be a surface. A $-1$ cycle $Z = \sum D_i \subset X$ is a rationally connected set of curves such that there is a sequence of maps 
\[
...
\end{tikzcd}
\]
Such that each map is a blowdown and the image of $Z$ is a smooth point.
\end{dfn}
...
\begin{tikzpicture}
	\begin{pgfonlayer}{nodelayer}
		...
		\node [style=none] (13) at (0.25, 2.75) {-1};
		\node [style=none] (14) at (1.5, 0.75) {0};
		\node [style=none] (15) at (-0.5, -1.75) {0};
		\node [style=none] (16) at (-2.75, -0.25) {-1};
	\end{pgfonlayer}
...
\begin{tikzcd}
X & Y \arrow[l] \arrow[r, "\pi"] & \mb{F}_{a_i}
\end{tikzcd}
\]
We now note that by construction on $Y$ with self intersection $a_i - k_1 -k_2$. This implies that cascade of $X$ is of length $L = a_i - k_1-k_2$. If $L < 0$ then this surface is not a \ldp\ surface.
