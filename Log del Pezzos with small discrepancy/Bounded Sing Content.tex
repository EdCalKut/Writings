...
We reference back to chapter 2 for a lot of the notation.
\label{sec!notation}
...
\section{Cascades}
Given a singularity with small discrepancy such that the minimal resolution is $a_1, \dots a_n$, there are a finite number of basic surfaces classified in the previous theorem.

\begin{thm}
Let $X_i$ be one of the surfaces constructed from $X_{a_i}$. If $i \neq 1,\, n$ then this admits a cascade of length $a_i + 3 - n$, if $i = 1, \, n$ then the cascade is of length $a_i + 4 - n$. 
...
\end{tikzpicture}
\]
This has $h^0(-K_X) \neq 0$, $-K_X^2 = \frac{3}{5}$, six $\frac{1}{3}(1,1)$ singularities and two $\frac{1}{5}(1,1)$ singularities. This is a complexity one surface.
\begin{lem}
If a complexity one surface admits a toric degeneration it is equivariant
\end{lem}
\begin{prop}
The above surface $X$ does not admit an equivariant toric degeneration.
\end{prop}
We can construct this as a toric complete intersection via cox rings and we see that it lies as a complete intersection in the toric variety given by the GIT quotient
\[
