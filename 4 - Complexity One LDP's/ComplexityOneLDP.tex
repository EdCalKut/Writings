...
\newtheorem{dfn}[thm]{Definition} % Change this to definition style
...
We can more generally describe what occurs with polyhedral divisors of the form $[\frac{a}{b}, \, \frac{c}{d}] \otimes P_0$ with $\frac{c}{d} < \frac{a}{b}$ with these fractions in reduced form. We note that the tail cone $\delta$ is  0 and so the dual tail cone is all of $M$. From the definitions we get the $\mathcal{O}_{\mathbb{A}^1_z}$ algebra $R = \bigoplus_{m \in M} R_m \chi^m$ where

\[
R_m = 
\begin{cases}
\mathcal{O}_{\mb{A}_z^1} ( \frac{mc}{d}P) & \text{if } m \geq 0,  \vspace{0.2cm} \\
\mathcal{O}_{\mb{A}_z^1} ( \frac{ma}{b}P) & \text{if } m \leq 0.
\end{cases}
\]

We can associate the monomial $z^u \chi^v$ with the lattice point $(u,v)$ inside a two dimensional monomial lattice. This gives rise to a cone $\sigma$. The set of monomials with poles of order at worst $\frac{a}{b}$ gives rise to the vector $(-a, b)$. Similarly the other side of the graded ring gives rise to the vector $(c, -d)$. These are boundary rays of $\sigma$, this means that as toric variety they can be described as the cone $(c,d)$, $(a,b)$ inside the lattice $N \cong \mathbb{Z}^2$ with torus action corresponding to $(1,0)$.  Hence we get a toric variety of the form $\frac{1}{r}(\alpha, \, \beta)$ where $r = ad-bc$ and $\alpha$ and $\beta$ are generators of the kernel of the matrix $M$ modulo $r$ where
\[
...
\]
This gives us the ring $\mb{C}[x, \frac{1}{x+1}, \frac{1}{x}\chi^2, \chi^1] = \mb{C}[u,v,w,t]/[v(u+1) = 1, \, uw = t^2]$. Hence this is an $A_1$ singularity. The calculations on the other 3 patches are the same, so we have taken the partial resolution of the $D_4$ singularity by extracting the trivalent curve. We can do this over any discrete valuation ring and this simplifies equations arising.
\end{ex}
...
\]
We note that the point $P_5$ is involved, however this does not contradict Lemma~ as the fiber over $P_1$ is the general fiber. So the number of general fibers is still bounded by four.
...
\[
\left[-\infty, \frac{1}{2} \right] \otimes P_1 + \left[ -\infty,0, \,1 \right] \otimes P_2 + [-\infty, -1, \, 0] \otimes P_3
\]
and
\[
...
\[
\left[-\infty, 0, \frac{1}{2} \right] \otimes P_1 + \left[-\infty, \frac{1}{2} \right] \otimes P_2 + [-\infty, -1, \, 0] \otimes P_3
\]
...
\[
\left[\frac{1}{4} \right] \otimes P_1 + \left[\frac{1}{2} \right] \otimes P_2 + [-1, \, 0] \otimes P_3
\]
...
\[
\left[\frac{1}{3} \right] \otimes P_1 + \left[\frac{1}{3} \right] \otimes P_2 + [-1, \, 0] \otimes P_3
\]
