...
\begin{proof}
Let $Y$ be the minimal resolution of $X$. Then $X$ admits an equivariant map to $\mb{F}_i$ as every $-1$ curve on $Y$ is torus invariant. Hence $Y$ is constructed by a non toric blowup of a toric surface. Considering the first non toric blowup, we are blowing up a point $P$ on a torus invariant curve $C$. Clearly $C$ is in the minimal resolution of $X$. This proofs the above lemma.
...
\begin{lem}
Let $X$ be a complexity one surface and let $D$ be an ample divisor. Then $D$ corresponds to a collection of functions $h_{P_i}$ with consistent linear part. Let $P \in \mb{P}^1$ be a point such that the fan over this point corresponds to a decomposition $0, \frac{p_1}{q_1}, \dots \, \frac{p_n}{q_n}$ with $p_i, \, q_i \geq 0$ then as a toric variety we have locally 
