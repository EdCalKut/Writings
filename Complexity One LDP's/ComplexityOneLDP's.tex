...
See \cite{Suss}
\end{proof}
This results in a non toric singularity having the restriction $(q_1, q_2, q_3)$ is one of $(2,2,m)$, $(2,3,3)$, $(2,3,4)$ or $(2,3,5)$. We now note that if the singularity is of index $l$ then we have the following bound $\sum \frac{p_i}{q_i} < 2l $\cite{Suss}[Section on classification]. From the easy toric observation EXPAND we get the central curve has of the minimal resolution has self intersection $\sum \lceil \frac{p_i}{q_i} \rceil $. By taking the partial resolution formed by just obtaining the trigonal curve we see  the remaining three cyclic quotient singularities have to have index dividing $l$. Via combining this information we get the bound that any curve $C$ in the minimal resolution of a log terminal singularity of index $l$ has to satisfy $C^2 \geq -2l$.
