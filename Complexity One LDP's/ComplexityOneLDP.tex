...
We can more generally describe what occurs with divisors of the form $[\frac{a}{b}, \, \frac{c}{d}] \otimes P_0$ with $\frac{a}{b} < \frac{c}{d}$. We note that the tail cone $\delta$ is always 0 and so the dual tail cone is all of $M$. From the definitions we get the ring $R = \bigoplus_{m \in M} R_m$ where
\[
R_n = 
\left\{
\begin{array}{cc}
...
Then at least one of the elliptic points, $P_1$, is a singularity.  We can describe $P_1$ locally by a cone $\sigma \subset N \cong \mb{Z}^2$ and a ray $v \in N$. Then $v$ corresponds to a lattice point on the minimal resolution of $\sigma$. Equivalently the minimal resolution of $X$ has at least one parabolic line.
\end{lem}
...
There are two infinite families of singularities of index $k$ corresponding to $A_n$ and $D_n$ singularities. There are then the sporadic cases corresponding to $E_6, \, E_7, \, E_8$.


We may assume that the we are dealing with a polyhedral fan defined $\mathcal{S}$ defined by the following 
\[\def\arraystretch{1.2}
\mathcal{S} = 
\begin{array}{cc}
...
We know that $\sum(\lceil w_i \rceil) \leq 3k$ via Lemma~\ref{Trivalent} and $w_1, \, w_2 >0$ and $w_3> -1$.

We first explicitly describe all possible values for $a_1^j, \, \dots , \, a_{n_j-1}^j$ for $j \geq 4$. This is equivalent to classifying toric varieties of the form $(0,\, 1),$ $(p_1, \, q_1),$ $\dots , $ $(p_{n_j-1}, \, q_{n_j-1})$, with $q \geq 0$. We now note that every choice of $(p_1, \, q_1)$ lies on a line going through one of the points $(-u,v)$ where $u$ divides $k$ and $0 < v < u$. The second condition guarantees that we have a $-1$ curve intersecting the parabolic curve. To stop double counting we insist $u$ and $v$ are coprime. The value $u$ is the index of the new singularity.


Taking a choice of $(u, v)$, we wish to bound the length of this line, which we will denote $L$. If $k = 1$ then it is bounded via Lemma~\ref{MaxLength}. In higher index this approach does not work. Instead we use the following: assume we pick a coordinate $(x,y) \in L$ then $(x,y) = (0, 1) + A(u, v-1)$. There is a unique transform $f \in SL_2(\mathbb{Z})$ that acts as a shear along the edge $(0,1)$ to $(x,y)$, hence up to sign $f(0,1) = (u, v)$ and $f((0,1) + A(u, v-1)) = (0,1) + (A+u)(u, v-1)$. This is a shear along the line $(0,0), (u, v-1)$, denote this line $L'$. From now on we pick the smallest possible value of $A$ upto this transform, this means $0< A < u$. There are only a finite number of fans $(x,y) = (x_0, y_0), \, (x_1, y_1), \, \dots , (x_n, y_n)$ such that $(x_n, y_n)$ is also above the line $L'$ and the fan is convex and of Gorenstein index dividing $k$. For a fixed $(x_n, y_n)$ then every possible choice of the next point lies on a line $L_{u'}$ where $u'$ divides $k$. This gives the following picture


As each of these lines intersect the line $L'$ after each application of the map $f$ their slopes become more negative. Hence the possible news values of 
...
\left[a_1^4, \, a_2^4,\,  \dots, a_{n_4 - 1}^4, \, 0 \right] \\
\vdots \\
\left[ a_1^{3k}, \, a_2^{3k}, \,  \dots, a_{n_{6k-1}}^{3k}, \, 0 \right] \\
\end{array}
\]
