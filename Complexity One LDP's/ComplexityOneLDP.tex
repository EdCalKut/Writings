...
\]
Once again in degree 0 we get the constant function. In degree one we get no functions. In degree 2 we get 2 functions $\frac{1}{x}$ and $\frac{1}{x-1}$ denote these $f$ and $g$. In degree 3 we get the function $\frac{1}{x(x-1)}$ denote this by $h$. We then have the relations $h^2 = fg(f-g)$. This gives rise to a $D_4$ singularity.  
...
\[
\bigoplus_{m \in M|_{\geq 0}} \mathcal{O}_{(\mb{P}^1 - P_1 - P_2)} \left( \frac{m}{2} P_0 + \frac{m}{2} P_1 + \frac{-m}{2} P_2 \right) \cong \bigoplus_{m \in M|_{\geq 0}} \mathcal{O}_{(\mb{A}^1 - P)} (\frac{m}{2}P_0)
\]
...
\item The unmarked cone $[-1, \, 1]$
\item The marked cone $1\otimes P_0 + \frac{1}{2} \otimes P_1 + \frac{-1}{2} \otimes P_2$ with tail cone $[0,\, \infty)$.
\item The marked cone $1\otimes P_0 + \frac{1}{2} \otimes P_1 + \frac{-1}{2} \otimes P_2$ with tail cone $(-\infty, \, 0]$.
\end{enumerate}
...
\begin{ex}\rm
Consider the polyhedral divisor defined by $(0,0) \otimes P_0 + (1, 1) \otimes P_1$ over $\mathbb{P}^1$. With tail cone $\delta = \langle (1,0) , \, (0, 1) \rangle$. We now insist that two dimensional cone is marked. However if we pick a ray of the tail fan, and calculate the dual we see that this does not satisfy of being semiample on the boundary, if the base curve was non affine.
